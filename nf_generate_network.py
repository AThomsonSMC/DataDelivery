...
def generate_edges(node_lists):
    ...
    while not valid:
        ...
        for user_node in node_lists[4]:
            total_cap = 0
            search_bounds = nf_utils.find_inbound_edges(user_node[0], node_lists[6], edge_boundaries, len(edge_list))
            for edge in edge_list[search_bounds[0]:search_bounds[1]]:
                if edge[2] == user_node[0]:
                    total_cap += edge[3]
            if total_cap < user_node[1]:
                print 'Invalid node: %s needs %s but can only get %s' %user_node[0], user_node[1], total_cap
                print 'Infeasible network.  Regenerating edges...'
                invalid = True
                attempts += 1
                if attempts > 10:
                    print 'Too many infeasible networks.  Please change the statics and try again.'
                    raise Exception 
        if not invalid:
            print 'Feasible network generated.  Continuing...'
            valid = True
